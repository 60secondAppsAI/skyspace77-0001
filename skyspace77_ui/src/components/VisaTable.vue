        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Visas</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalVisas = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalVisas">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Visa</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="VisaId" type="text" placeholder="Enter VisaId" v-model="visaToAdd.visaId"></base-input>
  <base-input label="VisaType" type="text" placeholder="Enter VisaType" v-model="visaToAdd.visaType"></base-input>
  <base-input label="IssueCountry" type="text" placeholder="Enter IssueCountry" v-model="visaToAdd.issueCountry"></base-input>
  <base-input label="ExpiryDate" type="text" placeholder="Enter ExpiryDate" v-model="visaToAdd.expiryDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="visas" :row-key="record => record.VisaId" :pagination="pagination"
...
import VisaPictureView from './VisaPictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'visaId',
          visible: true,
          scopedSlots: { customRender: 'visaId' },
          sorter: true
          //sorter: (a, b) => a.visaId - b.visaId,
          //sorter: (a, b) => a.visaId.localeCompare(b.visaId),
        },
        {
          title: 'Visa Type',
		dataIndex: 'visaType',
          visible: true,
          scopedSlots: { customRender: 'visaType' },
          sorter: true
          //sorter: (a, b) => a.visaType - b.visaType,
		...
		dataIndex: 'issueCountry',
          ...
          sorter: true
          //sorter: (a, b) => a.issueCountry - b.issueCountry,
          //sorter: (a, b) => a.issueCountry.localeCompare(b.issueCountry),
        },
        {
          title: 'Expiry Date',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} visas`,
      },
      ...
      visasTable: {
        columns: [...visasColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'visaId',           // Column to sort by
  ...
  methods: {
    ...
    async renderVisasTable() {
      ...
      for (let i = 0; i < this.visas.length; i++) {
        visasTableData.push({
          id: i,
          visaId: this.visas[i].visaId,
          year: this.visas[i].year,
          date: this.visas[i].date,
          competitionId: this.visas[i].competitionId,
          visaId: this.visas[i].visaId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-visas',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForVisasChanged', this.searchQuery);
		//this.renderVisasTable();
    },
	...
	async handleAddSubmitted() {
      this.modalVisas = false;

      const currentDate = new Date().getTime();
      this.visaToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.visaToAdd);
      console.log(jsonData);
      
      const res = await VisaService.addVisa(jsonData);
