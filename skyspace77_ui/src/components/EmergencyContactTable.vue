        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalEmergencyContacts = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalEmergencyContacts">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add EmergencyContact</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="EmergencyContactId" type="text" placeholder="Enter EmergencyContactId" v-model="emergencyContactToAdd.emergencyContactId"></base-input>
  <base-input label="ContactName" type="text" placeholder="Enter ContactName" v-model="emergencyContactToAdd.contactName"></base-input>
  <base-input label="ContactRelation" type="text" placeholder="Enter ContactRelation" v-model="emergencyContactToAdd.contactRelation"></base-input>
  <base-input label="ContactNumber" type="text" placeholder="Enter ContactNumber" v-model="emergencyContactToAdd.contactNumber"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="emergencyContacts" :row-key="record => record.EmergencyContactId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <EmergencyContactPictureView :emergencyContacts="emergencyContacts" />
          </div>
...
import EmergencyContactPictureView from './EmergencyContactPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    EmergencyContactPictureView
  },

  data() {
    return {
      modalEmergencyContacts: false,
        isTableView: true,

      columns: [
        {
          title: 'Emergency Contact Id',
		dataIndex: 'emergencyContactId',
          visible: true,
          scopedSlots: { customRender: 'emergencyContactId' },
          sorter: true
          //sorter: (a, b) => a.emergencyContactId - b.emergencyContactId,
          //sorter: (a, b) => a.emergencyContactId.localeCompare(b.emergencyContactId),
        },
        {
          title: 'Contact Name',
		...
		dataIndex: 'contactRelation',
          visible: true,
          scopedSlots: { customRender: 'contactRelation' },
          sorter: true
          //sorter: (a, b) => a.contactRelation - b.contactRelation,
          //sorter: (a, b) => a.contactRelation.localeCompare(b.contactRelation),
        },
        {
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} emergencyContacts`,
      },
      ...
      sortBy: 'emergencyContactId',           // Column to sort by
  ...
  methods: {
    ...
    async renderEmergencyContactsTable() {
      ...
      let emergencyContactsTableData = [];
      for (let i = 0; i < this.emergencyContacts.length; i++) {
        emergencyContactsTableData.push({
          id: i,
          emergencyContactId: this.emergencyContacts[i].emergencyContactId,
          year: this.emergencyContacts[i].year,
          date: this.emergencyContacts[i].date,
          competitionId: this.emergencyContacts[i].competitionId,
          emergencyContactId: this.emergencyContacts[i].emergencyContactId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-emergency-contacts',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForEmergencyContactsChanged', this.searchQuery);
		//this.renderEmergencyContactsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalEmergencyContacts = false;

      const currentDate = new Date().getTime();
      this.emergencyContactToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.emergencyContactToAdd);
      console.log(jsonData);
      
      const res = await EmergencyContactService.addEmergencyContact(jsonData);
