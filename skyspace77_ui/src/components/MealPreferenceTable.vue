        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalMealPreferences = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalMealPreferences">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add MealPreference</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="MealPreferenceId" type="text" placeholder="Enter MealPreferenceId" v-model="mealPreferenceToAdd.mealPreferenceId"></base-input>
  <base-input label="MealType" type="text" placeholder="Enter MealType" v-model="mealPreferenceToAdd.mealType"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="mealPreferences" :row-key="record => record.MealPreferenceId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <MealPreferencePictureView :mealPreferences="mealPreferences" />
          </div>
...
import MealPreferencePictureView from './MealPreferencePictureView.vue';


const mealPreferencesColumns = [
  "mealPreferenceId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    MealPreferencePictureView
  },

  data() {
    return {
      modalMealPreferences: false,
        isTableView: true,

      columns: [
        {
          title: 'Meal Preference Id',
		dataIndex: 'mealPreferenceId',
          visible: true,
          scopedSlots: { customRender: 'mealPreferenceId' },
          sorter: true
          //sorter: (a, b) => a.mealPreferenceId - b.mealPreferenceId,
          //sorter: (a, b) => a.mealPreferenceId.localeCompare(b.mealPreferenceId),
        },
        {
          title: 'Meal Type',
		dataIndex: 'mealType',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} mealPreferences`,
      },

      mealPreferences: [],
      mealPreferenceToAdd: {},

      mealPreferencesTable: {
        columns: [...mealPreferencesColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'mealPreferenceId',           // Column to sort by
  ...
  methods: {
    ...
    async renderMealPreferencesTable() {
      ...
      let mealPreferencesTableData = [];
      for (let i = 0; i < this.mealPreferences.length; i++) {
        mealPreferencesTableData.push({
          id: i,
          mealPreferenceId: this.mealPreferences[i].mealPreferenceId,
          year: this.mealPreferences[i].year,
          date: this.mealPreferences[i].date,
          competitionId: this.mealPreferences[i].competitionId,
          mealPreferenceId: this.mealPreferences[i].mealPreferenceId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-meal-preferences',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForMealPreferencesChanged', this.searchQuery);
		//this.renderMealPreferencesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalMealPreferences = false;

      const currentDate = new Date().getTime();
      this.mealPreferenceToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.mealPreferenceToAdd);
      console.log(jsonData);
      
      const res = await MealPreferenceService.addMealPreference(jsonData);
